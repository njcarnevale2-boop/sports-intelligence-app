...
def test_alternate_books_remain_truncated_but_all_books_exposed():
    rows = []
    rows.append({"sportsbook": "DraftKings", "point": 7.0, "price": -105.0, "edge_pp": 0.251, "ev_per_dollar": 0.512})
    for idx, book in enumerate(["FanDuel", "BetMGM", "Fanatics", "BetRivers", "Caesars"], start=1):
    ...
    assert any(row["book"] == "BetRivers" for row in all_books)
    assert any(row["book"] == "Caesars" for row in all_books)
